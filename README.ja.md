...
# 🦁 XXD Panel 073｜幾何学都市手帳

### 写真を、合理的な足場を備えた切断型微縮建築景観へ再構成する
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 073 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
**Panel 073** は、主体を切り開いた等角投影の微縮建築景観へ圧縮し、水中の大陸棚のような断面と空間階層をつくります。細密な足場は装飾ではなく、輪郭、折れ、切断面に沿って主体を包み、支え、建設し、修復し、測量する合理的な構造です。白いテクスチャ紙、元写真から導く鮮明な色、抑制的な編集注記が、建築図解の正確さと芸術ポスターの軽やかさを両立します。

### 向いている用途

- 正しい等角投影と切断空間が必要で、誤った遠近法による偽の立体ではなく、視点、層、切断面の関係を明確にしたい場合
- 足場に本当の論理を持たせ、水平・垂直部材と接続点を、包む、支える、修復する、測量する行為に結び付けたい場合
- ミニチュア模型、細線図解、白いテクスチャ紙、抑制された注記で建築研究ポスターや展覧会図録の秩序をつくりたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 073 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 等角投影の微縮建築 · 切断立方体 · 大陸棚断面 · 合理的な足場 · 手描き建築図解 · 白いテクスチャ紙
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 合理的な足場と主体が正確な空間システムをつくる、切断型等角投影の微縮建築ポスター |
| **識別しやすい特徴** | 白いテクスチャ紙上の微縮立体模型、明確な断面階層、構造節点に沿って展開する足場 |
| **入力の尊重** | 輪郭、構造、姿勢、物語関係を保ち、元写真の色相の手掛かりを明るく純化する |
...
```text
輪郭、構造、姿勢、物語関係を固定 → 正しい等角投影と断面空間を構築 → 大陸棚のような層をもつ微縮建築模型へ圧縮 → 輪郭、折れ、切断面に接続された足場を組む → 細線、平坦な色、明るくした元写真の色相で表現 → 白いテクスチャ紙に浮かべる → 抑制的な建築注記を加える
```

## 完成作の特徴

- 正しい等角投影、切断立方体、大陸棚のような断面で信頼できる空間階層をつくる。
- 足場は水平・垂直部材と接続節点で構成し、主体構造に沿って実際に展開する。
- 簡潔な幾何学、平坦な色面、細線、適度な構造細部で手描き建築図解をつくる。
- 元写真の色相の手掛かりを保ち、明るく純化し、灰みを除く。
- 白いテクスチャ紙、大きな余白、弱い影、少量の英語注記で研究ポスターの秩序をつくる。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-073-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 073 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-073 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-073 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-073 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-073 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 073 を適用し、共通設定は一度だけ解決
/xxd-panel-073 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-073`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-073-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-073-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全プロジェクト一覧

73 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 073 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–073） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
