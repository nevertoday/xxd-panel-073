...
# 🦁 XXD Panel 073｜기하학 도시 저널

### 사진을 합리적 비계가 있는 절단형 미니어처 건축 풍경으로 재구성합니다
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 073이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
**Panel 073**는 피사체를 잘라 연 등각 미니어처 건축 풍경으로 압축하고 수중 대륙붕 같은 단면과 공간 위계를 만듭니다. 촘촘한 비계는 장식이 아니라 윤곽, 전환부, 절단면을 따라 피사체를 감싸고 지지하고 건설하고 수리하고 측량하는 합리적 구조입니다. 흰색 질감 종이, 원본에서 가져온 선명한 색상 단서, 절제된 편집 표기가 건축 도해의 정확성과 아트 포스터의 가벼움을 함께 만듭니다.

### 이런 경우에 적합합니다

- 잘못된 원근으로 가짜 입체를 만들지 않고 시점, 층위와 절단면 관계가 명확한 올바른 등각·단면 공간이 필요할 때
- 수평·수직 부재와 연결 노드가 감싸기, 지지, 복원 또는 측량에 실제로 기여하는 논리적인 비계를 원할 때
- 미니어처 모델, 가는 선 도해, 흰 질감 종이와 절제된 주석으로 건축 연구 포스터와 전시 도록 같은 질서를 만들고 싶을 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 073로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 등각 미니어처 건축 · 절단 큐브 · 대륙붕 단면 · 합리적 비계 · 손그림 건축 도해 · 흰색 질감 종이
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 합리적 비계와 피사체가 정확한 공간 시스템을 이루는 절단형 등각 미니어처 건축 포스터 |
| **대표 특징** | 흰색 질감 종이 위 미니어처 입체 모형, 명확한 단면 위계, 구조 노드를 따라 펼쳐지는 비계 |
| **입력 존중 방식** | 윤곽, 구조, 자세, 서사 관계를 유지하고 원본 색상 단서를 밝고 맑게 정제함 |
...
```text
윤곽·구조·자세·서사 관계 고정 → 정확한 등각 및 단면 공간 구축 → 대륙붕 같은 층의 미니어처 건축 모델로 압축 → 윤곽·전환부·절단면에 연결된 비계 구성 → 가는 선·평면 색·밝게 조정한 원본 색상으로 표현 → 흰색 질감 종이에 띄우기 → 절제된 건축 표기 추가
```

## 완성작의 특징

- 정확한 등각 투영, 절단 큐브, 대륙붕 같은 단면으로 믿을 수 있는 공간 위계를 만듭니다.
- 비계는 수평·수직 부재와 연결 노드로 구성하고 피사체 구조를 따라 실제로 펼칩니다.
- 간결한 기하, 평면 색, 가는 선, 적절한 구조 세부로 손그림 건축 도해를 만듭니다.
- 원본 색상 단서를 유지한 뒤 밝고 맑게 정제하고 회색기를 제거합니다.
- 흰색 질감 종이, 넓은 여백, 약한 그림자, 소량의 영어 표기로 연구 포스터 질서를 만듭니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-073-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 073의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-073 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-073 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-073 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-073 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-073 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-073 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 073 적용, 공통 설정은 한 번만 확정
/xxd-panel-073 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-073`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-073-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-073-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트 목록

73개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 073까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–073) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
