<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 073 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 073 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
锁定轮廓、结构、姿态与叙事关系 → 转为正确等距视图和剖切式空间 → 压缩成微缩建筑模型与大陆架般层级 → 沿外轮廓、转折和切面建立有连接节点的脚手架 → 用细线、平面色块和源图提亮色相表达 → 悬浮于白色纹理纸大留白 → 加入克制建筑编辑标注
```

## 成品中最容易识别的风格特点

- 主体只保留最值得记住的身份、姿态、轮廓与叙事关系，不完整复刻复杂场景。
- 主体以正确等距视图、剖切立方体或水下大陆架般空间呈现，层级和透视必须成立。
- 脚手架由水平、垂直构件与连接节点组成，围绕结构真实展开，不是漂浮拼贴。
- 简洁几何、平面色块、纤细线条和适度结构细节共同形成手绘建筑图解感。
- 色彩保留原图色相线索，再提亮、提纯、去灰，不采用外层固定色板。
- 白色纹理纸、大留白、弱阴影和少量现代英文标注形成建筑研究与展览图录秩序。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-073-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–073） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
